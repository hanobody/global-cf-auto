...
func TestBuildAssetSummaryAndNoAlertCSVReport(t *testing.T) {
	...
	if summary.TotalDomains != 3 {
		t.Fatalf("TotalDomains = %d, want 3", summary.TotalDomains)
	}
	if summary.TotalCertificates != 1 {
		t.Fatalf("TotalCertificates = %d, want 1", summary.TotalCertificates)
	}
	if len(summary.SourceCounts) != 2 || summary.SourceCounts[0].Source != "acc-a" || summary.SourceCounts[0].Domains != 2 {
	...
	for _, want := range []string{"当前缓存域名: 3 个", "acc-a: 2 个域名", "acc-b: 1 个域名", "详细全量资产 CSV"} {
	...
	if !strings.Contains(string(content), "a.example.com") || !strings.Contains(string(content), "账户平台") {
